...
export default class Modes {
	...
	bubbleParticle( particle: Particle ){
		...
		if( interactivity.events.onhover.enable &&
				...
				if( ratio >= 0 && this.library.interactivity.mouseStatus === MouseInteractivityStatus.MOUSEMOVE ){
					...
					if (interactivity.modes.bubble.opacity !== particles.opacity.value) {
						if (interactivity.modes.bubble.opacity > particles.opacity.value) {
							...
							const opacity = particle.opacityValue - (particles.opacity.value - interactivity.modes.bubble.opacity) * ratio;
							if (opacity < particle.opacityValue && opacity >= interactivity.modes.bubble.opacity) {
								particle.bubbleOpacity = opacity;
							}
						}
		...
		} else if (interactivity.events.onclick.enable &&
			...
			if( this.bubble_clicking ){
				...
				const timeSpent = (new Date().getTime() - this.library.interactivity.mouseClickTime) / 1000;
				...
				const process = ( bubble_param: any, particles_param: any, p_obj_bubble: any, p_obj: any, id: any ) => {
					if( bubble_param != particles_param ){
						if( !this.bubble_duration_end ){
							if ( mouseDistance <= interactivity.modes.bubble.distance) {
								...
								if( obj != bubble_param ){
									let value: any = p_obj - ( timeSpent * ( p_obj - bubble_param ) / interactivity.modes.bubble.duration);
									if( id == 'size' )
							...
							if( p_obj_bubble != undefined ){
								let value_tmp: any = p_obj - (timeSpent * ( p_obj - bubble_param ) / interactivity.modes.bubble.duration);
	...
	repulseParticle( particle: Particle ){
		...
		if( interactivity.events.onhover.enable && 
			...
			if (this.repulse_clicking) {
				const repulseRadius = Math.pow(interactivity.modes.repulse.distance/6, 3);
				const {distance, distanceX, distanceY} = this.library.manager.getDistances(this.library.interactivity.mouseClickPosition, particle);
