...
type TSVGAbsoluteCoordinatesTypes = SVGPathSegArcAbs |
...
type TSVGRelativeCoordinatesTypes = SVGPathSegArcRel |
...
export class PolygonMask {
    ...
    parseSvgPathToPolygon(svgUrl?: string): Promise<Array<[number, number]>> {
        ...
        return this.parseSvgPath(svgUrl)
            .then(polygonData => {
                ...
                if (this.initialized)
                    this.library.manager.particlesRefresh();
    ...
    getRandomPointOnPolygonPath(): TPoint {
        ...
        const [x, y] = this.polygonRaw[Math.floor(Math.random() * this.polygonRaw.length)];
        return {
            x,
            y,
        };
    ...
    isPointInsidePolygon(point: TPoint): boolean {
        if (this.path2DSupported && this.polygonPath) {
            ...
            for (let i = 0, j = this.polygonRaw.length - 1; i < this.polygonRaw.length; j = i++) {
                const xi = this.polygonRaw[i][0], yi = this.polygonRaw[i][1];
                const xj = this.polygonRaw[j][0], yj = this.polygonRaw[j][1];
