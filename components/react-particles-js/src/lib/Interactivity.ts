...
export default class Interact {
	...
	onMouseClick() {
		...
		if (polygon.enable &&
			...
			const isInside = this.library.polygonMask.isPointInsidePolygon(this.mouseClickPosition);
	...
	linkParticles( p1: Particle, p2: Particle ): void{
		...
		if( distance <= lineLinked.distance ){
			const lineOpacity = lineLinked.opacity - ( distance / ( 1 / lineLinked.opacity ) ) / lineLinked.distance;
			if( lineOpacity > 0 ){
	...
	attractParticles(p1: Particle, p2: Particle): void {
		const {distance, distanceX, distanceY} = this.library.manager.getDistances(p1, p2);
