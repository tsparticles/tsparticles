...
This plan coordinates **4 independent features** targeting the `4.3.0` release.
...
> **Nota:** D ha priorità maggiore di B. Vedi Sequencing Strategy per ordine di esecuzione aggiornato.
...
```
D (BgMask Dynamic)        C (GIF Shape)             A (Background Canvas)     B (Fluid Interaction)
  plugin only                shape-image refactor       engine only               new package only
  no engine changes          shape-gif (new)            no deps on D/C            no deps on D/C
                             all bundle update
                             no deps on D/A/B

All four are INDEPENDENT — no shared files, no conflicting changes.
They can be implemented in any order or in parallel.
```

### Conflict matrix

| Area | D ∩ C | D ∩ A | D ∩ B | C ∩ A | C ∩ B | A ∩ B |
|------|-------|-------|-------|-------|-------|-------|
| Files | None | None | None | None | None | None |
| Types | None | D importa `BackgroundDrawContext` da engine (già esportato) | None | None | None | None |
| Engine | No | D e A toccano `background.*` ma A è engine, D è plugin (nessun conflitto) | No | A modifica engine; C solo tipi | C, B solo tipi engine | A modifica engine; B solo tipi |
| Bundle `all` | Not affected | C touches `bundles/all/` | Not affected | Not affected | Not affected | Not affected |
| Risk | None | None | None | None | None | None |

**Verdict:** All four are fully parallelizable. No merge conflicts expected.
The only integration point is `@tspictures/all` (feature C adds a dependency).
...
3. **Feature D (Background Mask Dynamic)** — pattern già validato, plugin isolato, alto valore wow
4. **Feature B (Fluid Interaction)** — most complex, new simulation code, needs tuning
...
**Priorità:** Più alta della Fluid Interaction (B).
...
| Scenario | Action |
|----------|--------|
| D not ready | Ship C + A (v2) + B as 4.3.0, defer D to 4.4.0 |
| C not ready | Ship D + A (v2) + B as 4.3.0, defer C to 4.4.0 |
| A (v2) not ready | Ship D + C + B as 4.3.0, defer A to 4.4.0 (or ship A v1 as-is) |
| B not ready | Ship D + C + A (v2) as 4.3.0, defer B to 4.4.0 |
| D+C not ready | Ship A (v2) + B as 4.3.0 |
| D+C+A not ready | Ship B alone as 4.3.0 |
| D+C+A+B not ready | No 4.3.0 — reassess scope |
