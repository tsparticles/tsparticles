# Feature F: Particle Modifier System

**Target:** `@tsparticles/engine` + `@tsparticles/interaction-external-bubble`

## Design

Replace the ad-hoc `particle.bubble` property override pattern with a generic **Particle Modifier** system. This lets any plugin (interactor, updater, etc.) temporarily override a particle's visual properties (radius, opacity, color, transform) without Particle.ts knowing about the specific plugin.
...
- `destroy()` resets `this.bubble.inRange = false`
- `init()` initializes `this.bubble = { inRange: false }`

The Bubble interactor (`Bubbler.ts`) directly mutates `particle.bubble.*` — this is an engine concern leaking into plugin territory.
...
- [ ] Bubble hover/click works identically to before
- [ ] `particle.bubble` no longer exists in engine code
- [ ] No `IBubbleParticleData` references remain in engine
- [ ] Modifier add/remove lifecycle works (add during hover, remove on leave)
- [ ] Particle destroy clears all modifiers
- [ ] Multiple modifiers can stack (priority ordering)
