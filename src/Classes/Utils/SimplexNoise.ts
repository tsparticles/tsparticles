export class SimplexNoise {
    ...
    private static readonly F4 = (Math.sqrt(5.0) - 1.0) / 4.0;
    private static readonly G4 = (5.0 - Math.sqrt(5.0)) / 20.0;

    private readonly p: Uint8Array;
    private readonly perm: Uint8Array;
    private readonly permMod12 = new Uint8Array();
    ...
    private static buildPermutationTable(random: () => number) {
        let i;
    ...
    public noise3D(xin: number, yin: number, zin: number): number {
        ...
        // Skew the input space to determine which simplex cell we're in
        const s = (xin + yin + zin) * SimplexNoise.F3; // Very nice and simple skew factor for 3D
        const i = Math.floor(xin + s);
        const j = Math.floor(yin + s);
    ...
    public noise4D(x: number, y: number, z: number, w: number): number {
        ...
        const s = (x + y + z + w) * SimplexNoise.F4; // Factor for 4D skewing
        const i = Math.floor(x + s);
        const j = Math.floor(y + s);
        const k = Math.floor(z + s);
        const l = Math.floor(w + s);
        const t = (i + j + k + l) * SimplexNoise.G4; // Factor for 4D unskewing
        ...
        const x1 = x0 - i1 + SimplexNoise.G4; // Offsets for second corner in (x,y,z,w) coords
        const y1 = y0 - j1 + SimplexNoise.G4;
        const z1 = z0 - k1 + SimplexNoise.G4;
        const w1 = w0 - l1 + SimplexNoise.G4;
        const x2 = x0 - i2 + 2.0 * SimplexNoise.G4; // Offsets for third corner in (x,y,z,w) coords
        const y2 = y0 - j2 + 2.0 * SimplexNoise.G4;
        const z2 = z0 - k2 + 2.0 * SimplexNoise.G4;
        const w2 = w0 - l2 + 2.0 * SimplexNoise.G4;
        const x3 = x0 - i3 + 3.0 * SimplexNoise.G4; // Offsets for fourth corner in (x,y,z,w) coords
        const y3 = y0 - j3 + 3.0 * SimplexNoise.G4;
        const z3 = z0 - k3 + 3.0 * SimplexNoise.G4;
        const w3 = w0 - l3 + 3.0 * SimplexNoise.G4;
        const x4 = x0 - 1.0 + 4.0 * SimplexNoise.G4; // Offsets for last corner in (x,y,z,w) coords
