...
async function updatePackageFile(destPath, name, description, repoUrl) {
  ...
  const packageContents = await fs.readFile(packagePath, "utf-8");
  const camelizedName = camelize(camelize(name, "-"), " ");
  const dashedName = dash(camelizedName);
  const descriptionRegex = /"tsParticles template preset"/g;
  const replacedDescriptionText = packageContents.replace(
      descriptionRegex,
      `"${description}"`
  );
 ...
 * @param {string} repoUrl
 */
async function updatePackageDistFile(destPath, name, description, repoUrl) {
