...
2. Load a base package (for example `@tsparticles/basic`) and call `loadMonochromeYellowsPalette` before `tsParticles.load(...)`
...
<script src="https://cdn.jsdelivr.net/npm/@tsparticles/palette-monochrome-yellows@4/tsparticles.palette-monochrome-yellows.min.js"></script>
```
...
(async engine => {
  ...
  await loadMonochromeYellowsPalette(engine);
